...
class PaperBroker:
    ...
    def get_ticker(self, markets: str):
        ...
        for market in requested:
            ...
            trade_volume = (
                float(candles[0].get("candle_acc_trade_volume", 0)) if candles else 0.0
            )
    ...
    def cancel_order(self, order_uuid):
        return {"status": "rejected", "reason": "no_open_order", "uuid": order_uuid}
    ...
    def buy_market(self, market: str, price: float, identifier: str | None = None):
        ...
        if current is None:
            self.positions[market] = Position(
                volume=filled_volume, avg_buy_price=fill_price
            )
        else:
            total_cost = (current.volume * current.avg_buy_price) + (
                filled_volume * fill_price
            )
            total_volume = current.volume + filled_volume
            self.positions[market] = Position(
                volume=total_volume, avg_buy_price=total_cost / total_volume
            )
    ...
    def sell_market(self, market: str, volume: float, identifier: str | None = None):
        ...
        if remaining_volume <= 0:
            ...
            self.positions[market] = Position(
                volume=remaining_volume, avg_buy_price=position.avg_buy_price
            )

...
def create_engine(broker=None, notifier=None, config=None, ws_client=None):
    ...
    if websocket_client is None and trading_config.mode not in {"paper", "dry_run"}:
        websocket_client = UpbitWebSocketClient(default_format=trading_config.ws_data_format)

    return TradingEngine(trade_broker, trade_notifier, trading_config, ws_client=websocket_client)

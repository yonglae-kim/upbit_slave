...
def check_sell(data, avg_buy_price):
    ...
    if avg_buy_price * 1.002 > float(data[0]['trade_price']):
        return False
    if macd['MACDDiff'].iloc[-2] > macd['MACDDiff'].iloc[-1]:
        return True
        ...
        for account in my_coins:
            ...
            if check_sell(data, float(account['avg_buy_price'])) or float(data[0]['trade_price']) < float(
                ...
                tele.sendMessage("SELL " + "KRW-" + account['currency'] + " " + str(data[0]['trade_price']) + " "
                                 + str(round(((float(data[0]['trade_price']) - float(account['avg_buy_price'])) / float(
                    account['avg_buy_price']))*100, 2)) + "%")
                time.sleep(5)

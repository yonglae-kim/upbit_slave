...
while 1:
    ...
    for account in my_coins:
        ...
        if check_sell(data):
            ...
            print("SELL", "KRW-" + account['currency'], str(account['balance']) + account['currency'], data[0]['trade_price'])

    tickers = apis.get_ticker(', '.join(list_krw_market))
    tickers.sort(key=lambda x: x['trade_volume'], reverse=True)

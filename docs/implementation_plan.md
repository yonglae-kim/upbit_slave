...
## 2단계: 데이터/유니버스 파이프라인
### 2-1. 종목 선별
- [x] `market/all`로 유니버스 구성
- [x] 24h 거래대금 상위 N1 선별
- [x] 상대 스프레드 필터 적용
- [x] 결측률(체결 없는 분봉 누락) 기준 적용
- [x] 최종 감시 N2(저사양 상한) 확정

### 2-2. 버퍼/정합성
- [x] 1m/5m/15m 고정 길이 링버퍼 구성
- [x] 캔들 누락 구간 정렬 정책 정의
- [x] 지표 계산 트리거를 "캔들 종료 시점"으로 제한
- 정책 요약: 누락 구간은 직전 종가 기반의 합성 캔들(`missing=True`)로 보정해 시계열 연속성을 유지한다.
- 전략 계산 영향 통제: 최신 캔들이 `missing`이거나 TF별 누락률이 `max_candle_missing_rate`를 초과하면 해당 마켓의 시그널 계산을 중단한다.
- 계산 트리거: 1m 최신 캔들 타임스탬프가 갱신된 "캔들 종료 시점"에만 전략 계산을 수행해 틱 단위 재평가를 차단한다.
...
## 5단계: 백테스트/검증
- [x] 캔들 최대 200개 제한 고려한 백필러 점검
- [x] 수수료+스프레드+슬리피지 비용 모델 반영
- [x] IS/OOS 분리 + 워크포워드 검증
- [x] 성과지표(CAGR, MDD, Sharpe, 체결률) 리포트

- 백필러 규칙: REST 캔들 호출은 회당 최대 200개(`MAX_CANDLE_LIMIT`)로 고정하고, 부족분은 가장 오래된 실캔들의 종가를 기준으로 `missing=True` 합성 캔들을 선행 구간에 채워 길이를 맞춘다.
- 비용 모델 강제: 매수/매도 체결가에 스프레드(half-spread)와 슬리피지를 반영하고, 수수료를 함께 차감해 백테스트 수익률을 계산한다.
- 검증 루프: `insample_windows`/`oos_windows` 기반 워크포워드 루프를 수행하고, 구간별 결과를 `backtest_walkforward_segments.csv`에 저장한다.
- 최소 지표 자동산출: 각 구간에서 CAGR, MDD, Sharpe, 체결률(fill_rate)을 계산하고 평균 요약을 출력한다.
